$( document ).ready(function() {
    ...
    function isPrivateIP(connection) {
    ...
    function validIPnPort(connection) {
        var parts = connection.split(':');
    ...
    $("._registerHost").click(function(){
        _error("_rhost", "");
        var connection1 = $("#ipname").val();
        var connection2 = $("#ipport").val();
        var connection = connection1 + ':' + connection2;
    ...
    $(".update_Host").click(function(){
        ...
        var _data = ow.methods.updateHost(connection).encodeABI();
        var _url = intrProvider+"?to="+owContract+"&value=0&data="+_data+"#send-transaction";
        openInNewTab(_url);
    });
    ...
    function init(){
        ...
        ow.methods.totalDomains().call(function(err1, _odt){
            $("._total_domains").text(_odt);
        });

        ow.methods.websiteUpdatesCounter().call(function(err1, _odt){
            $("._total_wupdates").text(_odt);
        });


        // ow.methods.totalSubscriber().call(function(err1, _odt){
        //     $("._total_subs").text(_odt);
        // });

        // ow.methods.totalHosts().call(function(err1, _odt){
        //     $("._total_hosts").text(_odt);
        // });
    }
